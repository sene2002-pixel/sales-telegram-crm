          ...
          if (r.chat_id)
            await this.reports.processing.run(r.source_key, () =>
              this.reports.notify(tx, r.chat_id, {
                text:
                  r.purpose === 'dialogue'
                    ? 'Не удалось разобрать запрос. Ничего не выполнено. Повторите.'
                    : r.purpose === 'contact'
                      ? 'Контакт не создан. Повторите команду.'
                      : 'Отчёт не обработан. Повторите в CRM.',
              }),
            );
        };
        if (this.diagnostics)
      ...
      try {
        ...
        await this.db.transaction(async (tx) => {
          ...
          if (rows.length && failed && report.chat_id)
            await this.reports.notify(tx, report.chat_id, {
              text: contact
                ? `Контакт не создан: ${message}`
                : report.purpose === 'dialogue'
                  ? `Не удалось разобрать запрос: ${message}`
                  : `Отчёт не обработан: ${message}. Повтор — в CRM.`,
            });
